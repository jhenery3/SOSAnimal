...
class LoginController extends \BaseController {
	...
	public function auth()
	{
		...
		if ($validator->passes())
		{
			if ( Auth::attempt(Input::only('email', 'password'), $remember = false) )
			{
				...
				{
					return Redirect::to('login')->with('warning', 'Su cuenta esta baneada !');
				}
			}
			else
			{
				return Redirect::to('login')->with('warning', 'Su combinación de nombre de usuario / contraseña era incorrecta !');
			}	
		}
		else
		{
			return Redirect::to('login')->with('warning', "Mensajes de error de validación ")->withErrors($validator);
		}
	}
	...
	public function reset()
	{
		...
		if ($validator->passes())
		{
			...
			{
				return Redirect::to('login')->with('warning', 'Este correo no esta registrado!');
			}
		}
		else
		{
		...
		return Redirect::to('login')->with('message', 'La contraseña fue restablecida! Consultar su correo electrónicol !');
	}	
	/* === END FORGOT PASSWORD === */
	...
	public function store()
	{
		...
		return Redirect::to('login')->with('message', "¡La cuenta fue creada! Inicia sesión con tu nombre de usuario y contraseña!");
	}
